...
namespace Nuclex.Audio.Metadata.Exceptions {
  ...
  [TestFixture]
  public class HandshakeFailedExceptionTest {
    ...
    public void TestDefaultConstructor() {
      HandshakeFailedException testException = new HandshakeFailedException();
    ...
    public void TestSerialization() {
      ...
      using(MemoryStream memory = new MemoryStream()) {
        HandshakeFailedException exception1 = new HandshakeFailedException(
          "Hello World"
        );
        ...
        Assert.IsInstanceOf<HandshakeFailedException>(exception2);
        Assert.AreEqual(
          exception1.Message, ((HandshakeFailedException)exception2).Message
        );
      }
    }
