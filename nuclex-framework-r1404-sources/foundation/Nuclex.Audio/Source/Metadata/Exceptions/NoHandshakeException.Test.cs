...
namespace Nuclex.Audio.Metadata.Exceptions {
  ...
  [TestFixture]
  public class NoHandshakeExceptionTest {
    ...
    public void TestDefaultConstructor() {
      NoHandshakeException testException = new NoHandshakeException();
    ...
    public void TestSerialization() {
      ...
      using(MemoryStream memory = new MemoryStream()) {
        NoHandshakeException exception1 = new NoHandshakeException(
          "Hello World"
        );
        ...
        Assert.IsInstanceOf<NoHandshakeException>(exception2);
        Assert.AreEqual(
          exception1.Message, ((NoHandshakeException)exception2).Message
        );
      }
    }
