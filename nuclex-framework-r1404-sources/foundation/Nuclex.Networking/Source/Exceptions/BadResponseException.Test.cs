...
namespace Nuclex.Networking.Exceptions {
  ...
  public class BadResponseExceptionTest {
    ...
    public void TestSerialization() {
      ...
      using(MemoryStream memory = new MemoryStream()) {
        BadResponseException exception1 = new BadResponseException(
          "Hello World"
        );
        ...
        Assert.IsInstanceOf<BadResponseException>(exception2);
        Assert.AreEqual(
          exception1.Message, ((BadResponseException)exception2).Message
        );
      }
    }
