  ...
  public class InputManagerTest {
    ...
    public void TestChangeUpdateOrder() {
      using (MockFactory mockery = new MockFactory()) {
        using (var manager = new InputManager()) {
          Mock<IUpdateableSubscriber> updateable = mockery.CreateMock<IUpdateableSubscriber>();
          manager.EnabledChanged += updateable.MockObject.EnabledChanged;
          manager.UpdateOrderChanged += updateable.MockObject.UpdateOrderChanged;

          updateable.Expects.One.Method(
            x => x.UpdateOrderChanged(null, null)
          ).WithAnyArguments();
