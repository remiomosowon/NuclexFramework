...
namespace Nuclex.Input {
  ...
  public class ControllerDetectorTest {
    ...
    public void TestMouseDetection() {
      Mock<IControllerDetectorSubscriber> subscriber = mockSubscriber();

      subscriber.Expects.One.Method(x => x.ControllerDetected(null, null)).With(
        NMock.Is.Anything, new ControllerEventArgsMatcher()
      );
    ...
    public void TestChatPad(PlayerIndex playerIndex) {
      ...
      subscriber.Expects.One.Method(x => x.ControllerDetected(null, null)).With(
        NMock.Is.Anything, new ControllerEventArgsMatcher(playerIndex)
      );
    ...
    private Mock<IControllerDetectorSubscriber> mockSubscriber() {
      Mock<IControllerDetectorSubscriber> subscriber = this.mockery.CreateMock<
        IControllerDetectorSubscriber
      >();

      this.detector.ControllerDetected += subscriber.MockObject.ControllerDetected;
