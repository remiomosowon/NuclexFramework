...
namespace Nuclex.UserInterface.Visuals.Flat.Renderers {
  ...
  internal class FlatChoiceControlRendererTest : ControlRendererTest {
    ...
    private void drawAndExpectState(string expectedState) {
      ...
      ).With(Is.StringContaining(expectedState), Is.Anything);

      MockedGraphics.Expects.One.Method(
        m => m.DrawString(null, RectangleF.Empty, null)
      ).With(Is.StringContaining(expectedState), Is.Anything, Is.EqualTo("Choice"));

      // Let the renderer draw the choice into the mocked graphics interface
      renderer.Render(this.choice, MockedGraphics.MockObject);
