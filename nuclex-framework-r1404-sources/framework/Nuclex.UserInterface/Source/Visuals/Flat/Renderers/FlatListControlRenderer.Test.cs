...
namespace Nuclex.UserInterface.Visuals.Flat.Renderers {
  ...
  internal class FlatListControlRendererTest : ControlRendererTest {
    ...
    private void drawAndExpectState() {
      ...
      ).WithAnyArguments().WillReturn(new RectangleF(0.0f, 0.0f, 10.0f, 10.0f));

      MockedGraphics.Expects.AtLeast(0).Method(
        m => m.DrawElement(null, RectangleF.Empty)
      ).WithAnyArguments();
      MockedGraphics.Expects.AtLeast(2).Method(
        m => m.DrawString(null, RectangleF.Empty, null)
      ).WithAnyArguments();

      // Let the renderer draw the list into the mocked graphics interface
      renderer.Render(this.list, MockedGraphics.MockObject);
