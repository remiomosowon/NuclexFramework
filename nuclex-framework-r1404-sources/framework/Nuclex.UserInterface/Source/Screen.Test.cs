...
namespace Nuclex.UserInterface {
  ...
  internal class ScreenTest {
    ...
    public void TestFocusChangeEvent() {
      Screen screen = new Screen(100.0f, 100.0f);
      Mock<IFocusChangeSubscriber> mockedSubscriber = mockSubscriber(screen);
    ...
    private Mock<IFocusChangeSubscriber> mockSubscriber(Screen screen) {
      Mock<IFocusChangeSubscriber> mockedSubscriber =
        this.mockery.CreateMock<IFocusChangeSubscriber>();
