...
namespace Nuclex { namespace Fonts { namespace Content {
  ...
  SpriteFontContent ^NuclexSpriteFontDescriptionProcessor::Process(
    ...
    FreeTypeFontRasterizer rasterizer(input, this->hinter);
    ...
    for each(wchar_t character in input->Characters) {
      ...
      if(characterBitmap != nullptr) {
        ...
        placement.X += 1; // ensures empty upper left corner in texture
        placement.Y += 1; // ensures empty upper left corner in texture
      ...
      // Calculate the bitmap's offset from the pen position based on whether
      // the user wants it placed relative to its upper end or to the base line.
      Point offset = rasterizer.GetOffset(character, this->origin);

      // Finally, add the offsets and cropping informations to the character
      Point advancement = rasterizer.GetAdvancement(character);
      content->Cropping->Add(
        Rectangle(offset.X, offset.Y, advancement.X, rasterizer.LineHeight)
      );

      // The SpriteBatch supports "pseudokerning" or "ABC width kerning", but I haven't
      // seen this ever being filled. It may be ignored by XNA's content processor or
      // it could just be that I've got no font on my system that provides this data.
