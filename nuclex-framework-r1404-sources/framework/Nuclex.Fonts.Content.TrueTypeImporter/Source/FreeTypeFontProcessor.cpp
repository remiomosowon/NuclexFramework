...
namespace Nuclex { namespace Fonts { namespace Content {
  ...
  FreeTypeFontProcessor::FreeTypeFontProcessor(
    ...
    FT_Error error = FT_Set_Char_Size(
      this->freeTypeFace, 0, static_cast<int>(fontDescription->Size * 64.0f), 96, 96
    );
    if(error)
      throw gcnew Exception("Could not set font size in FreeType");

    // Adjust the flags for FT_Load_Char() according to the desired hinter
    this->loadFlags = FT_LOAD_RENDER;
    switch(hinter) {
      case FontHinter::Disable: { this->loadFlags |= FT_LOAD_NO_HINTING; break; }
      case FontHinter::AutoHinter: { this->loadFlags |= FT_LOAD_FORCE_AUTOHINT; break; }
    }
  }
  ...
  void FreeTypeFontProcessor::LoadCharacter(wchar_t character) {
    ...
    FT_Error error = ::FT_Load_Char(this->freeTypeFace, character, this->loadFlags);
    if(error)
